...
func _ready() -> void:
	...
	# Si su ID está registrado en los datos guardados, desaparece del mapa.
	if id_unico != "" and id_unico in Guardado.objetos_recolectados:
		queue_free()
		
		# Realiza una segunda comprobación después de que termine de cargar el nodo.
...
func _recolectar_tarjeta() -> void:
	...
	# Muestra en la consola el ID de la instancia del inventario
	# y el contenido actual de las tarjetas almacenadas.
